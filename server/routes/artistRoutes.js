...
router.get('/artist/:id', (req, res) => {
    ...
    const query = `
        ...
        LEFT JOIN 
            (SELECT song_id, COUNT(*) as like_count FROM likes GROUP BY song_id) l 
    ...
    db.query(query, [artistId], (err, results) => {
        ...
        results.forEach(row => {
            if (row.album_id && !albumsMap.has(row.album_id)) {
            ...
            if (row.song_id) {
                ...
                album.songs.push({
                    ...
                    total_play_count: row.total_play_count || 0,
                    total_likes: row.total_likes || 0
                });
            }
        });

        artistInfo.albums = Array.from(albumsMap.values());
        res.json(artistInfo);
    });
});
...
// Add this new endpoint for fetching artist's songs
