...
const apiUrl = 'http://localhost:3360';
...
const ArtistReports = () => {
    ...
    // Update useEffect to include artistId dependency
    useEffect(() => {
        if (artistId) {
    ...
    // Add this useEffect to watch for date range changes
    useEffect(() => {
        if (artistId) {  // Only fetch if we have an artistId
            fetchPlatformActivity();
        }
    }, [dateRange.startDate, dateRange.endDate, artistId]); // Dependencies array includes both dates and artistId
